 ...
 * Copyright (C) 2016 syso <gh@ext.no-route.org>
 */
 ...
 */
public class TrotecSpeedy400TCP extends LaserCutter {
  ...
  tcp = new PrintStream(connection.getOutputStream(), true, "US-ASCII");
    ...
    for (JobPart p : job.getParts())
    {
      ...
      {
       // json.put("part3Draster", this.generatePseudoRaster3dGCode((Raster3dPart) p, p.getDPI()));
      }
      else if (p instanceof RasterPart)
      {
       // json.put("part2Draster",this.generatePseudoRasterGCode((RasterPart) p, p.getDPI()));
      }
    ...
    byte[] buf = this.generateShutdownCode();
        String hpglcode = new String(buf);
        
    json.put("shutdownCode", hpglcode.split("\\r?\\n"));
    out.close();
    tcp.println(hpglcode);
    if (connection != null)
    {
...
//      this.setFlipYaxis((Boolean) value);
  ...
  public LaserCutter clone() {
    TrotecSpeedy400TCP clone = new TrotecSpeedy400TCP();
